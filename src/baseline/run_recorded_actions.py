...
def run_recorded_actions_on_emulator_and_save_video(sess_id, instance_id, run_index):
    ...
    tdf = pd.read_csv(_SCRIPT_DIR / f"session_{sess_id}" / f"agent_stats_{instance_id}.csv.gz", compression='gzip')
    ...
    env_config = {
            ...
            'action_freq': 24, 'init_state': str(_PROJECT_ROOT / 'saves' / 'has_pokedex_nballs.state'), 'max_steps': max_steps, #ep_length,
            'print_rewards': False, 'save_video': True, 'fast_video': False, 'session_path': sess_path,
            'gb_path': str(_PROJECT_ROOT / 'ROM_INPUT' / 'PokemonRed.gb'), 'debug': False, 'sim_frame_dist': 2_000_000.0, 'instance_id': f'{instance_id}_recorded'
    }
