"""
V2 Gym environment for Pokemon Red.

Uses coordinate-based exploration (visited tile counting) instead of KNN frames.
Observation is a dict: grayscale screen stack, HP, Fourier-encoded level,
badges, event flags, local exploration map, and recent actions.
Policy: MultiInputPolicy (SB3 PPO).
"""
...
class RedGymEnv(Env):
    """
    V2 Pokemon Red environment with structured (dict) observations.

    Key differences from Baseline:
    - Exploration via coordinate counting (no KNN index, lower memory)
    - Dict observation space with separate channels for game state
    - Fourier-encoded level sum for richer level representation
    - Stuck penalty: negative reward if revisiting same tile too often
    """
    ...
    def __init__(self, config=None):
        ...
        # Dict observation: multiple modalities for MultiInputPolicy
    ...
    def update_recent_actions(self, action):
        """Roll action history and insert new action at position 0."""
    ...
    def get_game_state_reward(self, print_stats=False):
        ...
        Components: events, healing, badges, exploration bonus, stuck penalty.
        """
        state_scores = {
